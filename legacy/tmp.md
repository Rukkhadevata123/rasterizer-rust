...
重构不仅仅是为了“代码整洁”，更是为了**模拟真实图形学管线（Graphics Pipeline）的工作方式**。

目前的 `part_aa` 和 `part_ab` 虽然能跑，但它们是“硬编码”的。引入**着色器（Shader）**和**管线（Pipeline）**的概念，是为了让你的渲染器具备**可编程性**和**扩展性**。

让我们快速复习一下这两个核心概念，以及为什么它们对你的重构至关重要。

---

### 1. 什么是渲染管线 (The Graphics Pipeline)?

想象一条汽车组装流水线。不管造什么车，流程都是固定的：冲压 -> 焊接 -> 涂装 -> 总装。

在图形学中，这个流程就是：
1.  **顶点处理 (Vertex Processing)**: 把 3D 点从世界坐标搬到屏幕坐标。
2.  **光栅化 (Rasterization)**: 找出三角形覆盖了屏幕上的哪些像素。
3.  **片元处理 (Fragment Processing)**: 决定每个像素具体是什么颜色（光照、纹理）。
4.  **输出合并 (Output Merging)**: 写入深度缓冲和颜色缓冲。

**你现在的代码问题：**
你的 `Rasterizer::process_pixel` 函数里，把第 3 步（计算颜色）和第 4 步（写入缓冲）甚至第 2 步（插值）全部揉在了一个巨大的循环里。
*   如果你想加一个“卡通渲染”模式，你得在那个大循环里加一堆 `if (is_toon) { ... }`。
*   如果你想加“阴影贴图”，你又得加 `if (is_shadow_pass) { ... }`。

**重构后的目标：**
管线负责流程控制（遍历三角形、插值），但**不关心具体怎么算颜色**。
...
*   **顶点着色器 (Vertex Shader)**:
    *   **输入**: 一个顶点（位置、法线、UV）。
    *   **输出**: 裁剪空间位置（给光栅化器用） + 传递给片元的数据（Varying）。
    *   **你的旧代码**: 散落在 `transform_geometry` 函数里。

*   **片元着色器 (Fragment Shader)**:
    *   **输入**: 插值后的数据（Varying）。
    *   **输出**: 最终颜色。
    *   **你的旧代码**: 散落在 `process_pixel` 里的 `calculate_lighting` 和 `compute_pbr`。

**引入 Shader Trait 的好处：**
你可以定义不同的 Shader 结构体，比如 `PhongShader`, `PBRShader`, `DepthShader`。
当你想画阴影贴图时，你只需要把 `DepthShader` 塞给管线；当你想画物体时，塞 `PBRShader`。**管线代码一行都不用改。**
...
1.  **解耦算法与逻辑**：
    *   **光栅化器 (Rasterizer)** 只负责数学：判断点是否在三角形内，计算重心坐标。它不需要知道什么是“光”，什么是“材质”。
    *   **着色器 (Shader)** 只负责物理：计算光照公式。它不需要知道三角形是怎么画在屏幕上的。

2.  **性能优化**：
    *   目前的 `RenderSettings` 字符串解析发生在每一帧甚至每个像素。
    *   重构后，Shader 在渲染前初始化一次（解析好参数），渲染时只进行纯数学计算。

3.  **功能扩展**：
    *   未来如果你想做 **法线贴图 (Normal Mapping)**，你只需要写一个新的 Shader，或者修改现有的 Shader，完全不动光栅化器的核心代码。

---

### 总结：我们正在做什么？

我们正在把你的代码从 **"一个能画图的程序"** 变成 **"一个通用的图形引擎"**。

*   **旧代码**: `draw_scene()` 函数里写死了“先算坐标，再算光照，再画背景”。
*   **新代码**: `draw_scene()` 变成：
    1.  配置管线状态（开启深度测试，设置视口）。
    2.  绑定 Shader（比如 PBR Shader）。
    3.  发射绘制命令（Draw Call）。

这就是为什么我们要先写 `math` 库，接下来写 `shader` 接口，最后才把原来的逻辑搬进去。

准备好继续了吗？下一步我们将定义 **Shader Trait** 和 **光栅化所需的插值逻辑**。
