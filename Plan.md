...
- [ ] Finish canonical error ownership before removing the root `error` module. Public render/present target construction and configuration validation now use structured errors, and `ConfigError` lives in `io::config`; move or re-export the remaining glTF, image-output, scene-asset, and resolve/tonemap errors from their owning `io`, `scene`, or `render` modules, then make binary-only application, benchmark, and window errors local to the executable.
