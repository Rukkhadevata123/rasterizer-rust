...
- [x] `54a3e9b` (`expand final product cleanup plan`): reopened the cleanup gate after auditing shipping prose, public exports, command/cache boundaries, public error signatures, test names, source comments, and tracked benchmark history as a final product rather than as a migration branch. The audit found three code-level issues beyond wording: binary/tooling roots remain semver-public, public `RenderGeometry::IndexedTriangle` exposes the private `cache_vertices` policy, and caller-reused `ObjectBindingId` values can alias distinct draw contexts in the transformed-vertex cache. It also records public `String` errors, 14 phase-prefixed glTF tests, change-history/decorative/speculative comments, migration narration in active docs, and phase-numbered benchmark artifacts. Added ordered acceptance tasks for each issue, final version/package cleanup, external Plan archival, and negative scans. Warnings-denied Clippy remains clean; no correctness or performance run was needed because this slice changes only the tracked plan.
- [x] `13bc7a8` (`hide internal render geometry`): removed `RenderGeometry` and the generic geometry-recording method from the public `render` façade, leaving `draw_mesh` as the supported external draw entry point while retaining the indexed transparent path solely inside the built-in pipeline. Tightening visibility exposed the unconstructed `Triangle` variant and its backend branches, which were deleted instead of suppressed or retained as compatibility code. The private indexed path still derives cache eligibility from mesh reuse and preserves existing transparent behavior. All 160 release Rust tests, the custom-shader example, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, and `unreachable_pub` pass. No performance matrix was run because the active execution and cache algorithms are unchanged.
- [x] `1a6773a` (`make cache identities caller-safe`): removed `ObjectBindingId` from the public façade and changed `set_draw_bindings` to allocate an internal monotonically increasing identity for every binding update within the recorded pass, so external callers can no longer alias different contexts in the transformed-vertex cache. Replaced caller-chosen `VertexProgramId::from_pass_index` values with process-unique tokens from `VertexProgramId::new`; copying one token across raster-only pipeline variants still preserves deliberate cross-pipeline vertex reuse, while independently created programs cannot collide. Built-in passes, external tests, the custom-shader example, and the API guide use only the safe construction path, with no compatibility constructor retained. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc/doctests, `unreachable_pub`, example execution, and package-content checks pass. All 14 benchmark hashes remained stable. A noisy five-frame baseline-first matrix and ten-frame candidate-first matrix disagreed on short 16-worker outliers; baseline-first 100-frame checks of the two remaining suspects passed at +4.64% for high transparency and -6.72% for image background.
- [x] `1cce027` (`structure target and config errors`): replaced public string failures from `RenderTarget`, `MainHdrTarget`, `PresentBuffer`, and `Config::validate` with `RenderTargetError`, `PresentBufferError`, and `ConfigValidationError`; moved `ConfigError` to its canonical `io::config` module without an old-path alias. Added allocation-free public target-dimension validation so the bundled application no longer reaches into the private framebuffer, kept inconsistent framebuffer-layout errors private, and preserved the CLI diagnostic contract through typed error sources in `ApplicationError`. Existing validation tests now match variants rather than parsing display strings, without adding tests. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, `unreachable_pub`, the custom-shader example, and package-content checks pass. No performance matrix was run because successful allocation and rendering work are unchanged.
- [x] `306d6af` (`move library errors to owning modules`): moved `GltfError` and `PrimitiveContext` into the glTF loader, `ImageOutputError` into image output, `AssetError` into scene loading, and `ResolveTonemapError` into the private pass implementation with canonical public re-exports from `io`, `scene`, and `render`. Updated the external glTF tests to use `io::GltfError` and removed every library-side dependency on the root `error` module without retaining old-path aliases. The root module now contains only executable application, benchmark, and window errors. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, `unreachable_pub`, the custom-shader example, and package-content checks pass. No performance matrix was run because this slice changes type ownership and import paths only.
- [x] `110523b` (`make executable tooling binary-local`): moved the application loop, benchmark runner, combined application errors, and window input behind the binary crate boundary by declaring them only from `main.rs`; removed their four public module declarations from `lib.rs` without compatibility exports. The binary now consumes only the public `render`, `scene`, and `io` library roots, and rustdoc exposes exactly those three top-level modules. Updated the API guide and README structure to record the resolved boundary. All 160 release Rust tests remain, with 10 tooling tests now running under the binary target; 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, `unreachable_pub`, the custom-shader example, and package-content checks also pass. No performance matrix was run because runtime execution and data layout are unchanged.
- [x] `deba991` (`clean gltf test boundaries`): renamed all 14 remaining phase-prefixed glTF integration tests after their observable import, validation, or error-context behavior, so test output no longer carries development chronology. Narrowed the test-only `Mesh::create_test_triangle` helper from unrestricted visibility to `pub(super)`, which is sufficient for its two scene-loader test call sites; no alternate helper or compatibility path was added. The suite remains at 160 release Rust tests plus 9 benchmark-script tests. Formatting, warnings-denied Clippy, release checks, rustdoc, and `unreachable_pub` pass. No performance matrix was run because production code is unchanged.
- [x] `b5c54bc` (`make queue submission infallible`): re-ran production reachability and low-reference audits after the public-boundary cleanup. The retained private execution chain is singular: command recording produces ordered phases and draw packets, `GraphicsQueue` alone owns the software backend in production, and the backend alone drives the rasterizer; direct backend construction remains confined to internal invariant tests. The audit found and removed one stale public shell: `RenderError` only wrapped `CommandError`, while `GraphicsQueue::submit` had no failure branch and could never construct it. Submission now directly returns `SubmissionReport`; built-in passes, public integration tests, the packaged example, and API documentation use the exact contract with no compatibility wrapper. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, `unreachable_pub`, example execution, and package-content checks pass. No performance matrix was run because generated work, execution order, and data layout are unchanged.
- [x] `6abefdf` (`clean production comments`): audited production code and the packaged example, removing 63 net lines of change-history notes, obvious vertex/triangle labels, default-value restatements, speculative wording, redundant method summaries, and signature-duplicating argument/return templates. Replaced the few comments that carried real information with concise current contracts for shader outputs, UV-density mip selection, tangent handedness, coordinate/depth conventions, ground winding, light direction/attenuation, and decoded two-channel images. Concurrency ownership, transparent order, cache identity, clipping/numeric stability, color space, and PBR energy comments remain. Negative scans for the named comment smells pass. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, and `unreachable_pub` pass. No performance matrix was run because only comments changed.
- [x] `e766784` (`document current rendering api`): rewrote the README, crate documentation, and long-form rendering guide as documentation for the shipping API, removing branch status, migration chronology, the former API inventory, compatibility narration, and release checklists while retaining the implemented shader, command, attachment, ownership, cache, timing, and transparent-order contracts. Renamed the guide to `docs/rendering-api.md` and updated its README link and Cargo package inclusion atomically. Negative scans leave only semantic `RenderPhase` and `PhaseSubmissionReport` identifiers in source. All 160 release Rust tests, the custom-shader example, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, `unreachable_pub`, and package-content verification pass. No performance matrix was run because production rendering code is unchanged.
- [x] `0d9d835` (`document current benchmark contract`): rewrote `benchmarks/README.md` around the implemented schema-v2 producer, scenario runner, and comparator instead of development progress. The guide now defines timing ownership and inclusive submission totals, exact scenario/output behavior, matching environment and workload metadata, independent hash validation, the adjacent same-machine 5% mean gate, symmetric reruns and reverse-order noise handling, inconclusive outcomes, and reviewed exception requirements. It explicitly rejects repository-stored measurements from another machine as a comparison denominator. All 160 release Rust tests, 9 benchmark-script tests, formatting, warnings-denied Clippy, release checks, rustdoc, and `unreachable_pub` pass. No performance matrix was run because only documentation changed.
...
- [x] Keep application, benchmark, error aggregation, and window-input tooling behind the binary boundary. The library exposes only `render`, `scene`, and `io`; no `#[doc(hidden)]` or compatibility exports remain.
- [x] Remove backend cache policy from the public command surface. `RenderGeometry` and its generic recording method are pipeline-private; external callers record meshes through `draw_mesh`, while the built-in transparent path alone selects indexed-triangle cache eligibility.
- [x] Make binding identity safe for external callers. Render passes now issue private binding identities, and public vertex-program tokens are process-unique while remaining copyable for deliberate reuse across raster-only pipeline variants. Caller-chosen pass indexes and the public `ObjectBindingId` no longer exist.
- [x] Keep library errors under `io`, `scene`, or `render`, and application, benchmark, and window errors local to the executable. The library has no root `error` module, old-path aliases, or library-side dependency on binary error aggregation.
...
- [x] Name glTF tests after observable behavior rather than development chronology, and keep test-only helpers at the narrowest visibility needed.

### Final product and repository hygiene

- [x] Keep one production execution chain from typed command recording through `GraphicsQueue`, private ordered phases/draw packets, the software backend, and the rasterizer. Direct backend construction is test-only; the unused submission-error shell and all known compatibility or alternative execution paths are absent.
- [x] Keep production and example comments limited to current non-obvious contracts such as color space, handedness, cache identity, clipping, transparent order, numeric stability, and exclusive-band ownership; change-history, decorative, speculative, and signature-duplicating comments are absent.
- [x] Rewrite shipping documentation as documentation for the current product, not for a branch or migration. README, crate/module rustdoc, and `docs/rendering-api.md` contain no branch status, target-version narrative, former API inventory, migration checklist, or compatibility story; Cargo packaging and links use the version-neutral guide path.
...
- [x] Keep README, the rendering API guide, examples, module documentation, and benchmark documentation limited to the implemented API and current operational contracts. Remaining phase-numbered benchmark result artifacts are not active guidance and are covered by the repository-hygiene gate above.
- [x] Run formatting, Clippy with warnings denied, release checks, the complete 160-test Rust suite, all 9 benchmark-script tests, rustdoc, `unreachable_pub`, and the full release benchmark matrix.
- [x] Confirm deterministic hashes and the 5% full-frame mean budget against a fresh same-machine `f306e62` baseline in both run orders. All 14 scenario hashes match and every mean comparison passes without an exception.
