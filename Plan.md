...
- [ ] Begin with reusable immutable pipeline values.
- [ ] Add a cache only if lookup/construction or mirrored/cull/wireframe variants make it useful.
- [ ] If added, define a complete `PipelineKey` and test equality/hash coverage for every state field.
- [ ] GUI cull/wireframe changes should select a pipeline variant, not mutate a pipeline already referenced by recorded work.
