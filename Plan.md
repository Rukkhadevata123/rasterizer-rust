...
- [ ] Make `Rasterizer`, `SoftwareRasterBackend`, `RenderPhase`, `DrawPacket`, preparation scratch, and band-bin types private implementation details behind the `render` façade. The implementation types are now non-public; this item remains open until the `render` façade replaces the current broad module exports.
