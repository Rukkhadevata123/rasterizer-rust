...
- [ ] Finish canonical error ownership by making the remaining application, benchmark, and window errors local to the executable, then remove the public root `error` module. Library errors now live under `io`, `scene`, or `render`, with no old-path aliases or library-side dependency on `error`.
